...
/*
   ...
   exits. Temporary files and directories are useful for this purpose since they don’t pollute the
...
func main() {
	// The easiest way to create a temporary file is by calling ioutil.TempFile. It creates a file
	// and opens it for reading and writing. We provide "" as the first argument,
	...
	// The file name starts with the prefix given as the second argument to ioutil.TempFile and the
	// rest is chosen automatically to ensure that concurrent calls will always create different
	// file names.
	fmt.Println("Temporary filename:", f.Name())
	...
	// ioutil.TempDir’s arguments are the same as TempFile’s, but it returns a directory name
...
/*
   Test:
   ----
   $ go run 63_temporary_files_and_directories/temporary_files_and_directories.go
   Temporary filename: /tmp/sample440190691
   Temp dir name: /tmp/sampledir722545894
*/
